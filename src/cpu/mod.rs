...
fn decode(&mut self, rom_array: &Vec<u8>) {
    ...
    let _msg = match opcode {
        ...
        0x05 | 0x15 | 0x25 | 0x35 | 0x0d | 0x1d | 0x2d | 0x3d =>
            self.dec_reg(opcode),
        /* STORE AND INC/DEC */
        0x22 => self.store_and_update(memory::Operation::Increment),
        0x32 => self.store_and_update(memory::Operation::Decrement),
        /* LOAD AND INC/DEC */
        0x2a => self.load_and_update(memory::Operation::Increment),
        0x3a => self.load_and_update(memory::Operation::Decrement),
        /* LD SINGLE WORD */
        0x06 | 0x16 | 0x26 | 0x36 | 0x0e | 0x1e | 0x2e | 0x3e =>
            self.load_word_imm(opcode, code_bytes),
        /* LD DOUBLE WORD */
        0x01 | 0x11 | 0x21 | 0x31 => self.load_dword_imm(opcode, code_bytes),
        /* LD IMM ADDR */
        0xd0 => self.ret_cond(jump::Condition::NoCarry),
        0xea => self.store_imm_addr(code_bytes),
        /* LD INDIRECT ADDR */
        0xe2 => self.store_a_indirect_c(),
        0xf2 => self.load_a_indirect_c(),
        /* LDH (a8), A */
        0xe0 => self.store_a_mem(code_bytes),
        /* LDH A, (a8) */
        0xf0 => self.load_a_mem(code_bytes),
